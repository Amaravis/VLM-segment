...
from verl.utils.cpu_system import get_system_stats
...
def compute_cpu_system():
    cpu_usage, mem_usage = get_system_stats()
    print("cpu_stats: {} / {}".format(cpu_usage, mem_usage))
    return {
        **{"cpu_stats/cpu_usage":float(cpu_usage)},
        **{"cpu_stats/mem_usage":float(mem_usage)},
        }
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        unified_qa_model=self.config.data.unified_qa_model,
        ...
        load_from_disk_flag=getattr(self.config.data, "load_from_disk_flag", False),
        use_prompt_examples=getattr(self.config.data, "use_prompt_examples", False),
    )
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "images" in test_batch.non_tensor_batch.keys():
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
                    non_tensor_batch_keys=["raw_prompt_ids", "images"],
                )
            elif "pixel_values" in test_batch.non_tensor_batch.keys():
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
    ...
    def _save_checkpoint(self):
        ...
        # save dataloader for resume training (xhm)
        ...
        # local_latest_checkpointed_iteration = os.path.join(
        #     local_global_step_folder, "latest_checkpointed_iteration.txt"
        # )
        
        with open(local_latest_checkpointed_iteration, "w") as f:
            f.write(str(self.global_steps))
        
        clean_old_ckpts(self.config.trainer.save_checkpoint_path, f"global_step_{self.global_steps}")

    def _load_checkpoint(self):
        if self.config.trainer.load_checkpoint_path is None and self.config.trainer.resume_mode in ("disable", "disbale"):
            return 0

        checkpoint_folder = self.config.trainer.save_checkpoint_path  # TODO: check path
        ...
        ckpt_path = global_step_folder if global_step_folder is not None else self.config.trainer.load_checkpoint_path
        # ckpt_path = self.config.trainer.load_checkpoint_path         
        print(f"Load from checkpoint: {ckpt_path}")
        actor_path = os.path.join(ckpt_path, "actor")
        critic_path = os.path.join(ckpt_path, "critic")
        self.actor_rollout_wg.load_checkpoint(
            actor_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load
        )
        if self.use_critic:  # false for grpo (yjz)
    ...
    def fit(self):
        ...
        for _ in range(self.config.trainer.total_episodes):
            for batch_dict in self.train_dataloader:
                ...
                # pop those keys for generation
                if "images" in batch.non_tensor_batch.keys():
                    # New Qwen2.5-VL path: text + images for vLLM
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=["raw_prompt_ids", "images"],
                    )
                elif "pixel_values" in batch.non_tensor_batch.keys():
                    # Backward compatibility for older image datasets
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                        non_tensor_batch_keys=["pixel_values", "image_grid_thw", "raw_prompt_ids", "images"],
                    )
                ...
                metrics.update(compute_cpu_system()) # update memory usage of cpu

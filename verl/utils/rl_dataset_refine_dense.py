...
def build_initial_prompt_text_coarse_v2(
    question: str,
    question_type: str,
    width: int,
    height: int,
    ...
    # Soft heuristic ranges (NOT constraints). Tuned so that at 1920x1080 it “suggests”
    # the kind of coarse widths/heights you mentioned, but stays resolution-relative.
    w_lo, w_hi = int(0.06 * width), int(0.15 * width)      # ~115–288 at 1920
    h_lo, h_hi = int(0.10 * height), int(0.26 * height)    # ~108–280 at 1080
...
This is the COARSE stage: prioritize COVERAGE (high recall).
- It is better to include extra background than to miss any part of the target.
- Your box should fully contain the relevant object/region + a safety margin.
- If the target might be small or uncertain, avoid tiny boxes. Rule of thumb for small targets:
  a coarse box often has a side length on the order of ~{w_lo}–{w_hi} px (width) or ~{h_lo}–{h_hi} px (height),
  but make it larger if the object/region is larger.

Tasks:
1) Predict a coarse bounding box (integer pixel coords) that covers the needed object/region.
2) Answer the question.
3) Choose TWO integer points inside the object/region (e.g., near center + another salient part).

Output format (MUST FOLLOW EXACTLY):

think:
Write 2–4 short numbered steps describing what you see and why your coarse box covers it.
...
{{
  ...
  "response": "...",
  "decision": "refine",
  "reason": "short: why this box covers the region and why these points are inside it"
}}

Rules:
- "bbox_2d" uses THIS image’s pixel coordinates (no normalization).
- All bbox/point coordinates MUST be integers with 0 <= x < {width}, 0 <= y < {height}.
- points_2d must be exactly two points and inside the object/region.
- decision is always "refine" here.
- JSON only (double quotes), no extra text after JSON.
- {response_hint}
"""
    return QUESTION_TEMPLATE
...
def build_refine_prompt_text_v2(
    question: str,
    question_type: str,
    width: int,
    height: int,
    ...
    # Step-aware guidance (soft): earlier steps allow bigger shifts/tightening,
    # final step is small trims or stop.
    if step < max_steps:
        tighten_hint = "Try to tighten noticeably if there is clear background margin."
        shrink_pct = "about 15–45% area reduction when possible (by moving one or more edges inward)"
        stop_bias = 'Prefer "refine" unless the box is already close to optimal.'
        shift_bias = (
            "If the object/region is off-center in the current box, shift (translate) the box toward it "
            "(left/right/up/down) while also tightening."
        )
    else:
        tighten_hint = "This is the last step: do small trims only, or stop if already tight."
        shrink_pct = "about 0–25% area reduction (small edge trims), otherwise stop"
        stop_bias = 'Prefer "stop" if the box already fits well.'
        shift_bias = (
            "Only do small recentering shifts if needed; otherwise keep position stable and stop."
        )
    ...
    You see the full image. Use GLOBAL context, but treat the current bbox as your starting point.

    This is refinement step {step}/{max_steps}: your goal is to move toward a TIGHTER box than the current one.

    Core behaviors (refinement):
    - Prefer SHRINKING and SHIFTING to reduce background while keeping the whole object/region.
    - Shifting is encouraged: recenter the object/region in the box (object near box center), not just a symmetric shrink.
    - Adjust edges independently (left/right/top/bottom): shrink MORE on sides that contain mostly background.
    - {shift_bias}
    - Continuity (soft): the new box should usually still overlap the current box, but it may shift substantially if the current box is misaligned.
    - Never "teleport" to a totally different region unless the current box contains little/no relevant content.
    - Only expand if needed to avoid cutting off the object/region.

    Tightening guidance (soft):
    - {tighten_hint}
    - If you choose "refine", aim for {shrink_pct}.
    - Target end state: object/region fully inside with ~10–20% extra background.
    - {stop_bias}

    Tasks:
    1) Decide "refine" or "stop".
    2) Output the refined bbox (integers).
    3) Answer the question.
    4) Choose TWO integer points inside the object/region (e.g., center-ish + another salient part).

    Output format (MUST FOLLOW EXACTLY):

    think:
    Write 2–5 short numbered steps: what’s wrong with the current box (if anything),
    how you recenter it (shift) and tighten it (which edges move), where the two points go, and why.
    ...
    "response": "...",
    "decision": "refine" | "stop",
    "reason": "short: why this adjustment + why these points + why refine/stop"
    }}

    Rules:
    - bbox coords MUST be integers with 0 <= x < {width}, 0 <= y < {height}.
    - points_2d must be exactly two integer points inside the object/region (and inside the box).
    - decision must be exactly "refine" or "stop" (lowercase).
    - JSON only (double quotes), no extra text after JSON.
    - {response_hint}
    """
...
def build_initial_prompt_text_no_example_v4(
    question: str,
    question_type: str,
    width: int,
    height: int,
    ...
    """
    Coarse stage: encourage coverage-first coarse boxes.
    In think:, the model does normal reasoning AND samples 2–4 candidate bboxes (interleaved),
    then commits to one in 'choose:'.
    Also includes soft pixel-range heuristics derived from image resolution (not hard constraints).
    """
    response_hint = _response_hint(question_type, options)
    ...
    # Soft heuristic ranges (NOT constraints). For 1920x1080 this roughly suggests:
    # width ~115–288 px, height ~108–280 px
    w_lo, w_hi = int(0.06 * width), int(0.15 * width)
    h_lo, h_hi = int(0.10 * height), int(0.26 * height)
...
This is the COARSE stage: prioritize COVERAGE (high recall).
- Better to include extra background than to miss any part of the relevant object/region.
- Include a safety margin; avoid overly tight/tiny boxes if uncertain.
- Rule of thumb (soft, not a constraint): when the target might be small/uncertain, a coarse box often has
  side lengths on the order of ~{w_lo}–{w_hi} px (width) or ~{h_lo}–{h_hi} px (height). Use larger boxes if needed.

Tasks:
1) Predict a coarse bounding box (integer pixel coords) that covers the object/region needed to answer.
2) Answer the question.
3) Choose TWO integer points inside the object/region useful for segmentation.

Output format (VERY IMPORTANT, MUST FOLLOW EXACTLY):

think:
Start with normal reasoning about the image and question (what object/region matters, where it is, why).
During your reasoning, also sample and consider 2–4 candidate bounding boxes. Use this exact notation
whenever you propose a candidate:
  cand: [x_min, y_min, x_max, y_max]
Then commit to ONE final choice using this exact notation:
  choose: [x_min, y_min, x_max, y_max]
Your reasoning must include at least two 'cand:' lines before 'choose:'.
Do NOT output JSON in think:.
...
{{
  ...
  "response": "...",
  "decision": "refine",
  "reason": "short explanation of why this is a good coarse box and point choice"
}}

Rules:
- bbox_2d uses THIS image's pixel coordinates (no normalization).
- bbox_2d coordinates MUST be integers with 0 <= x < {width}, 0 <= y < {height}.
- points_2d MUST be exactly two integer points inside the object/region.
- decision is always "refine" at this initial step.
- The JSON bbox_2d MUST match the 'choose:' bbox exactly.
- JSON must use double quotes for all keys and string values.
- Do NOT output any text after the JSON object.
- {response_hint}
"""
...
def build_refine_prompt_text_v4(
    question: str,
    question_type: str,
    width: int,
    height: int,
    ...
    """
    Refine stage: encourages shift + shrink, prevents no-op refine.
    In think:, the model should do normal reasoning AND sample 2–4 candidate refined bboxes
    (interleaved), then commit to a final chosen bbox.
    """
    x1, y1, x2, y2 = current_bbox
    bbox_token = f"<box>({int(x1)},{int(y1)}),({int(x2)},{int(y2)})</box>"
    response_hint = _response_hint(question_type, options)
    ...
    # Soft step-aware hint (not a constraint)
    if step < max_steps:
        step_hint = "Earlier refinement: shifting + noticeable tightening is encouraged if there is background margin."
        stop_bias = 'Prefer "refine" unless the box is already close to optimal.'
    else:
        step_hint = "Final refinement: small trims or stop if already tight."
        stop_bias = 'Prefer "stop" if the box already fits well.'
...
You always see the full image. Use GLOBAL context, but treat the current bbox as your starting point.

Refinement step {step}/{max_steps} intent:
- Improve box quality by reducing background and recentering on the object/region (shift + shrink).
- Keep the full object/region inside the box.

Core behaviors:
- Prefer SHRINKING and SHIFTING (recenter the object/region in the box, not just symmetric shrink).
- Adjust edges independently (left/right/top/bottom): shrink more on sides that are mostly background.
- Avoid teleporting: do not jump to a totally different region unless the current box contains little/no relevant content.
- If decision="refine", the final bbox MUST change noticeably (shift and/or shrink). If no change is needed, use decision="stop".
- Only expand if needed to avoid cutting off the object/region.
- {step_hint} {stop_bias}
...
2) Output the refined bbox (integers).
3) Answer the question.
4) Choose TWO integer points inside the object/region.

Output format (VERY IMPORTANT, MUST FOLLOW EXACTLY):

think:
Start with normal reasoning about what the object/region is and how the current box is misaligned
(too much background on one side, off-center, too big/small, etc.).
During your reasoning, also sample and consider 2–4 candidate refined boxes. Use this exact notation
whenever you propose a candidate:
  cand: [x_min, y_min, x_max, y_max]
Then commit to ONE final choice using this exact notation (include decision in your reasoning text):
  choose: [x_min, y_min, x_max, y_max]
Do NOT output JSON in think:.
    ...
    def _build_prompt_body(
        ...
        if stage == "initial":
            if self.use_prompt_examples:
                ...
                return build_initial_prompt_text_no_example_v4(
                    question=question,
                    question_type=question_type,
                    width=image_width,
                    height=image_height,
        ...
        if self.use_prompt_examples:
            ...
            return build_refine_prompt_text_v4(
                question=question,
                question_type=question_type,
                width=image_width,
                height=image_height,
